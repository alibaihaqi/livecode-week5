 ...
 */
 ...
 function oeprasiMatriks(arr1, arr2, operator) {
   var grup1 = arr1;
   var grup2 = arr2;
   var operasiMatematika = operator;
   var hasil = [];

  if (grup1.length === grup2.length) {
    for (var i = 0; i <= grup1.length - 1; i++) {
      if (grup1[i].length === grup2[i].length) {
          var grupGabungan = [];
          for (var k = 0; k <= grup1[i].length - 1; k++) {
            if (operasiMatematika === '+') {
              grupGabungan.push(grup1[i][k] + grup2[i][k]);
            }
            else {
              grupGabungan.push(grup1[i][k] - grup2[i][k]);
            }
          }
          hasil.push(grupGabungan);
      }
      else {
        return 'Invalid Array';
      }
